...
/**
 * 확정산 Item 한 건을 독립 트랜잭션으로 처리
 * 외부 환율 조회는 이 컴포넌트에 진입하기 전에 끝내야 한다.
 */
@Component
...
public class SettlementTransactionExecutor {
  ...
  public void execute(SettlementJoinDTO target, BigDecimal finalRate) {
    ...
    KrwExchangeDTO exchange = krwExchangeMapper.selectExchangeByIdForUpdate(target.getExchangeId()).orElseThrow(() -> {
    ...
    if (target.getSellOrderStatus() == null || !SellOrderStatus.EXECUTED.equals(target.getSellOrderStatus())) {
  ...
  private void validateTarget(SettlementJoinDTO target) {
    if (target == null || target.getItemId() == null || target.getExchangeId() == null || target.getAccountId() == null) {
