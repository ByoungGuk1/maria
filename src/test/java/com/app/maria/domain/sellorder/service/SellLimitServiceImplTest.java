...
class SellLimitServiceImplTest {
    ...
    void validateSellLimitThrowsWhenAccountNotFoundByInboundId() {
        when(sellLimitMapper.selectAccountByInboundId(1L)).thenReturn(Optional.empty());

        assertThatThrownBy(() -> sellLimitService.validateSellLimit(1L, new BigDecimal("1000")))
                .isInstanceOf(SellOrderException.class)
    ...
    void validateSellLimitThrowsWhenAccountLimitNotFound() {
    ...
    void validateSellLimitThrowsWhenCiHashNotFound() {
        when(sellLimitMapper.selectAccountByInboundId(1L)).thenReturn(Optional.of(100L));
